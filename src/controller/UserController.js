...
class UserController {
  ...
  static async signup(request, response) {
    try {
      const { email, password, name, age, address } = request.body;
      if (!email || !password || !name || !age)
        return response
          .status(422)
          .json({ message: "Please enter all valid data" });
      ...
      if (user)
        return response
          .status(201)
          .json({ status: 201, message: "User has been created", user });
  ...
  static async getUser(request, response) {
    try {
      ...
      return response
        .status(200)
        .json({ status: 200, user, message: "User found" });
    } catch (error) {
  ...
  static async updateUser(request, response) {
    try {
      const { uuid } = request.user;
      const { password, age, name, email, address } = request.body;
      ...
      if (result)
        return response
          .status(200)
          .json({ status: 200, message: "Recieved posts", result });
  ...
  static async signin(request, response) {
    try {
      ...
      return response
        .status(200)
        .json({ status: 200, token, message: "User loggedin successful" });
    } catch (error) {
      console.log(error);
      response.status(500).json({ status: 500, error });
    }
